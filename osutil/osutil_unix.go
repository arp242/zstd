...
func Readable(s os.FileInfo) (bool, error) {
	...
	if !ok {
		return false, fmt.Errorf("osutil.Readable: assert to syscall.Stat_t failed; platform not supported?")
	}
	...
	if err != nil {
		return false, fmt.Errorf("osutil.Readable: %w", err)
	}
...
func Writable(s os.FileInfo) (bool, error) {
	stat, ok := s.Sys().(*syscall.Stat_t)
	if !ok {
		return false, fmt.Errorf("osutil.Writable: assert to syscall.Stat_t failed; platform not supported?")
	}
	...
	gids, err := os.Getgroups()
	if err != nil {
		return false, fmt.Errorf("osutil.Writable: %w", err)
	}
	for _, gid := range gids {
		if int(stat.Gid) == gid {
			return perm.Group.Read, nil
